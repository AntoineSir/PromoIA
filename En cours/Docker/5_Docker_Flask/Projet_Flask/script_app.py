...
mariadb_host = os.environ.get('DB_HOST', 'localhost')
mariadb_user = os.environ.get('DB_USER', 'root')
mariadb_pwd = os.environ.get('DB_PASSWORD', 'password')
...
def see_users():
    try: #on essaye de se connecter à la bdd
        app_db = mariadb.connect(host=mariadb_host, user=mariadb_user, password=mariadb_pwd, database="app_db")
        curseur = app_db.cursor()
